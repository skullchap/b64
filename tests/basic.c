...
int main() {
	...
	printf("The result for encoding of string is wrong, left: %s, right: %s", result, expected_result);
	return 1;
    } else {
    ...
    if(strcmp(reencoded, input)) {
        printf("The result for decoding of string is wrong, left: %s, right: %s", reencoded, input);
        return 1;
    } else {
